...
allowed-tools: ["Bash(${CLAUDE_PLUGIN_ROOT}/scripts/setup-autopoiesis.sh)"]
...
if [ -f /tmp/active_promise.md ]; then
  PROMISE=$(grep '^completion_promise:' /tmp/active_promise.md | sed 's/completion_promise: *//' | sed 's/^"\(.*\)"$/\1/')
